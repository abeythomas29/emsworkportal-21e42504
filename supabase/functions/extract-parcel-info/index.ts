...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const sys = `You read shipping labels from Indian couriers. Identify the tracking/AWB/docket number and the courier company. Reply ONLY in JSON.`;
    const userText = `Extract from this shipping label.
Return strict JSON: {"tracking_id":"...","courier":"...","confidence":"high|medium|low"}.
Courier must be one of: Delhivery, Bluedart, DTDC, Ekart, Ecom Express, XpressBees, Shadowfax, India Post, DHL, FedEx, ST Courier, Professional, Trackon, Gati, Maruti, Other.
